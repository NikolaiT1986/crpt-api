...
4. **Обработчик ошибок**: Проверка обязательных полей документа и выбрасывание `NullPointerException` (с сообщением) или
...
Преимущества реализации `SlidingWindowRateLimiter` по сравнению с Semaphore:

- В отличие от Semaphore, который ограничивает количество одновременных потоков, SlidingWindowRateLimiter
  ограничивает количество запросов за определённый интервал времени, что позволяет точнее контролировать нагрузку.
- Использование ReentrantLock и Condition позволяет избежать фоновых потоков, обеспечивая более эффективное
  использование ресурсов.
- Можно регистрировать собственные конвертеры форматов через
  `registerConverter(String format, DocumentConverter converter)`, в том числе заменить JSON-конвертер MANUAL на свой до
  заморозки реестра, которая происходит при 1-м вызове метода создания документа.
- Легко настраиваемая стратегия с параметрами для времени и количества запросов.
- Логика работы с метками времени запросов упрощает тестирование и расширение системы.